...
3. 디자인 가이드: 프로젝트 루트의 `DESIGN_GUIDE.md` 및 `CLAUDE.md`를 100% 엄격 준수
   - 폰트: Pretendard
   - 메인 컬러: KEPCO Dark Navy (`#0e3a6c`), Primary Blue (`#1976d2`), Energy Orange (`#ff7a00`)
   - 헤더: 56px 고정 헤더, 로고는 흰색 필 컨테이너 내 28px 높이 고정
   - 푸터: 38px 고정, 다크 네이비 배경, 공식 저작권 중앙 정렬 (`&copy; 2026 KEPCO Energy Solution Co., Ltd. All Rights Reserved.`). **⚠️ 임시 데모/프로토타입 안내 문구 절대 금지**
...
- **로고 비정상 크기 버그 해결**: `index.html` 상단에서 원본 해상도로 거대하게 노출되던 문제를 `style.css` 내 `.portal-logo-img` 절대 고정 규격(28px) 및 다크 네이비 그라디언트 헤더 적용으로 완전 해결.
